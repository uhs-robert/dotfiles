...
Item {
    ...
    // hearts, pixel (stepped, integer motion), lcd (pixel-styled, then the glyph blinks twice at the end) or levelup (inverted flash, then pixel sparkles).
    ...
    readonly property int duration: root.levelup ? (root.nudge ? 500 : 800) : (root.lcd ? 1300 : 1100) - (root.nudge ? 400 : 0)
    ...
    Repeater {
        model: root.levelup ? [] : root.nudge ? [{ dx: 0, delay: 100 }] : [{ dx: -3, delay: 120 }, { dx: 3, delay: 260 }, { dx: 0, delay: 400 }]
    ...
    Text {
        ...
        y: -height / 2
        text: root.glyph
        color: root.inverted ? root.dark : root.color
        font.family: root.font_family
