...
PanelWindow {
    ...
    Rectangle {
        ...
        readonly property real header_height: codec_header.item ? codec_header.item.implicitHeight : title_tab.visible ? title_tab.height : 0

        y: root.slide * (1 - root.reveal)
        opacity: root.reveal
        width: Math.max(body.implicitWidth + pad_x * 2, title_tab.visible ? title_tab.width : 0, codec_header.item ? codec_header.item.implicitWidth : 0)
