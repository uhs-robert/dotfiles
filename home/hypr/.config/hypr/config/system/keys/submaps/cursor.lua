-- home/hypr/.config/hypr/keys/submaps/cursor.lua
...
hl.define_submap(CURSOR.name, function()
  ...
  -- stylua: ignore start
  Bind.bind(SUBMAP.windows,    function() swap_submap(SUBMAP.windows.name) end)
  Bind.bind(SUBMAP.resize,     function() swap_submap(SUBMAP.resize.name) end)
  Bind.bind(SUBMAP.screenshot, function() swap_submap(SUBMAP.screenshot.name) end)
  Bind.bind(SUBMAP.system,     function() swap_submap(SUBMAP.system.name) end)
  -- stylua: ignore end

  -- !--- Floating Mode: Cursor click/move (vimium style) ---
  -- stylua: ignore start
  Bind.run_swap_submap("F",         "wl-kbptr -o modes=floating,click -o mode_floating.source=detect", CURSOR.name, "Floating (Click)")
  Bind.run_swap_submap("SHIFT + F", "wl-kbptr -o modes=floating -o mode_floating.source=detect",       CURSOR.name, "Floating Cursor Move")
  Bind.run_then( "CTRL + F",  "wl-kbptr -o modes=floating,click -o mode_floating.source=detect", function() restore_cursor_config(CURSOR.defaults) end, "Floating (Click/Exit)")

  -- !--- Tiling Mode: Cursor click/move ---
  Bind.run_swap_submap("T",         "wl-kbptr -o modes=tile,click", CURSOR.name, "Tiling (Click)")
  Bind.run_swap_submap("SHIFT + T", "wl-kbptr -o modes=tile",       CURSOR.name, "Tiling Cursor Move")
  Bind.run_then( "CTRL + T",  "wl-kbptr -o modes=tile,click", function() restore_cursor_config(CURSOR.defaults) end, "Tiling (Click/Exit)")
  -- stylua: ignore end
  ...
  Bind.run("CTRL + A", "wlrctl pointer click left",   "Left Click (Exit)")
  Bind.run("CTRL + S", "wlrctl pointer click middle", "Middle Click (Exit)")
  Bind.run("CTRL + D", "wlrctl pointer click right",  "Right Click (Exit)")
  -- stylua: ignore end
